...
def _test(file='cs'):
    ...
    for feature in issel:
        for is_smote in isshingle:
            Y_median = []
            Y_iqr = []
            for i, filename in enumerate(filenamelist):
                Y = F_final[filename][feature][is_smote]
                Y_median.append(np.median(Y))
                Y_iqr.append(np.percentile(Y, 75) - np.percentile(Y, 25))

            line, = plt.plot(X, Y_median, label="median_" + feature + "_" + is_smote)
            plt.plot(X, Y_iqr, "-.", color=line.get_color(), label="iqr_" + feature + "_" + is_smote)
    plt.xticks(X, filenamelist)
    plt.ylabel("F score")
